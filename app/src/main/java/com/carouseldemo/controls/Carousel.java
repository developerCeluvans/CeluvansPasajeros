 ...
 */
public class Carousel extends CarouselSpinner implements
	...
	// Retrieve names
	// int namesForItems = arr.getResourceId(R.styleable.Carousel_Names,
	// -1);
	...
	adapter.SetImages(images, null, mUseReflection);
	...
	for (int j = 0; j < childCount; j++) {
	    CarouselItem view = (CarouselItem) getAdapter().getView(j, null,
		    null);
	    if (i == 0)
		view.setDrawn(false);
	...
	mCamera.translate(((CarouselItem) child).getItemX(),
		((CarouselItem) child).getItemY(),
		((CarouselItem) child).getItemZ() - 100);
	...
	public void SetImages(TypedArray array, TypedArray namess,
		boolean reflected) {
    ...
    // private void Calculate3DPosition(CarouselItem child, int diameter, float
    // angleOffset){
    //
    // angleOffset = angleOffset * (float)(Math.PI/180.0f);
    //
    // float x = - (float)(diameter/2 * android.util.FloatMath.sin(angleOffset))
    // + diameter/2 - child.getWidth()/2;
    // float z = diameter/2 * (1.0f -
    // (float)android.util.FloatMath.cos(angleOffset));
    // float y = - getHeight()/2 + (float) (z *
    // android.util.FloatMath.sin(mTheta));
    //
    // child.setItemX(x);
    // child.setItemZ(z);
    // child.setItemY(y);
	...
	float x = -(float) ((diameter + (metrics.widthPixels / 1.8)) / 2 * Math
		.sin(angleOffset)) + diameter / 2 - child.getWidth() / 2;

	float z = diameter / 2 * (1.5f - (float) Math.cos(angleOffset));

	float y = (float) (-(getHeight() / 2.5f) - (z * Math.sin(-60)));
	...
	case DisplayMetrics.DENSITY_LOW:
	    ...
	    x = (-(float) ((diameter + (metrics.widthPixels / 8)) / 2 * Math
		    .sin(angleOffset)) + diameter / 2 - child.getWidth() / 2);
	...
	case DisplayMetrics.DENSITY_MEDIUM:
		...
		x = (-(float) ((diameter + (metrics.widthPixels / 4.5)) / 2 * Math
			.sin(angleOffset)) + diameter / 2 - child.getWidth() / 2);

	    }

	    y = (float) (-(getHeight() / 2.5f) - (z * Math.sin(-25.1)));
	...
	case DisplayMetrics.DENSITY_HIGH:
	    ...
	    x = (-(float) ((diameter + (metrics.widthPixels / 3.5)) / 2 * Math
		    .sin(angleOffset)) + diameter / 2 - child.getWidth() / 2);
	...
	case DisplayMetrics.DENSITY_XHIGH:
	    ...
	    //Log.e("DESITITY", "xhigh");

	    x = (-(float) ((diameter + (metrics.widthPixels / 3)) / 2 * Math
		    .sin(angleOffset)) + diameter / 2 - child.getWidth() / 2);
	...
	if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
	    ...
	    child.setItemY(y + 100);

	    //Log.e("ORIENTATION", "land");

	} else {

	    child.setItemY(y - 4);

	    //Log.e("ORIENTATION", "port");
	}
	...
	return (getWidth() - Carousel.this.getPaddingLeft() - Carousel.this
		.getPaddingRight()) / 2 + Carousel.this.getPaddingLeft();
    }
	...
	int extremeItemPosition = motionToLeft ? Carousel.this.getCount() - 1
		: 0;
	...
	for (int i = 0; i < getAdapter().getCount(); i++) {
	    ...
	    CarouselItem child = (CarouselItem) getAdapter().getView(i, null,
		    null);
